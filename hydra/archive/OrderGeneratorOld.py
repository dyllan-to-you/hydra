...
def dbTest(db):
    idsum = 0
    db.execute("SELECT DISTINCT timestamp FROM aroon_entry ORDER BY timestamp ASC")
    timestamps = [ts for (ts,) in db.fetchall()]
    for current_time in tqdm(
        timestamps,
        total=len(timestamps),
        unit="tick",
    ):
        entry_ids = get_aroon_from_db(db, current_time, "entry")
        idsum += sum(entry_ids)
    print("DB Total Ids:", idsum)
    return idsum
...
def pdbTest(db):
    entries_df = pd.read_sql(
        "SELECT id, timestamp from aroon_entry",
        db,
        parse_dates="timestamp",
    )
    entries_df = entries_df.set_index(["timestamp"])
    total_ids = 0
    for entry_idx, (entry_id, entries) in enumerate(tqdm(entries_df.groupby(level=0))):
        total_ids += entries["id"].sum()
    print("Pnad Database Total Ids:", total_ids)
    return total_ids


@timeme
def recompressParq(file, compression, compression_level=None):
    parq = pd.read_parquet(file)
    newFile = file.split(".")
    newFile[-1:1] = [str(x) for x in [compression, compression_level] if x is not None]
...
@timeme
def parqTest(file="aroon_entry.ts.parquet"):
    entries_df = pd.read_parquet(file)
    entries_df = entries_df.set_index(["timestamp"])
    print("MEMORY", entries_df.memory_usage(index=True).sum())
    total_ids = 0
    for entry_idx, (entry_id, entries) in enumerate(tqdm(entries_df.groupby(level=0))):
        total_ids += entries["id"].sum()
        if entry_idx >= 144337.7:
            break
    print("Parq Total Ids:", total_ids)
    return total_ids


@timeme
def get_aroon_from_db(db, current_time, direction):
...
if __name__ == "__main__":
    try:
        ...
        # parqTest(recompressParq("aroon_entry.ts.parquet", None))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "SNAPPY"))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "brotli", 1))
        parqTest(recompressParq("aroon_entry.ts.parquet", "brotli", 2))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "brotli", 3))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "brotli", 6))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "brotli", 9))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "ZSTD"))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "LZ4"))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "LZ4", 7))
        # parqTest(recompressParq("aroon_entry.ts.parquet", "LZ4", 14))
